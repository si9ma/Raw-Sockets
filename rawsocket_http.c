								  ...
								  int *resp_offset, char *ret_buffer)
{
	...
	while (tokenizer != NULL && total_payload_processed <= res_len)
	{
		...
		if (chunk_size > (res_len - total_payload_processed))
		{
			...
			*resp_offset = strlen(tokenizer) + (res_len - total_payload_processed) + 2;
			break;
		}
						  ...
						  uint16_t ret_buffer_len)
{
	...
	do
	{
		...
		resp_offset = 0;
	} while (content_length > 0 || (chunked_transfer && !end_of_chunked_response));
...
int main(int argc, char **argv)
{
	...
	char src[REQ_LENGTH], dst[REQ_LENGTH], dst_file_name[REQ_LENGTH] = {0}, get_request_url[REQ_LENGTH] = {0};
	...
	while (strncmp(get_request_url, "", 256) != 0)
	{
		...
		if ((ret = send_data(get_command, strlen(get_command))) < 0 || ret != strlen(get_command))
		{
